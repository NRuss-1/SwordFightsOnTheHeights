...
local maxffcount = 1
local maxexplosioncount = 1

function ExplodeKill(character, humanoid, attacker)
	...
	if ffcount >= maxffcount then
		return
	end
	
	if explodecount >= maxexplosioncount then
		return
	end
		...
		if (right_arm ~= nil) then
			...
			if (joint ~= nil and (joint.Part0 == sword or joint.Part1 == sword)) then
				if humanoid.Health > damage then
					...
					ExplodeKill(humanoid.Parent, humanoid, vPlayer)
				end
			end
		end
